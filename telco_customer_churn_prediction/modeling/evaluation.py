...
def churn_evaluation_report(
    ...
    # Logic: Low Risk (Loyalists) are eager to accept (High rate).
    #        High Risk (Churners) are angry/checked out (Low rate).
    accept_rate_min=0.10,  # How likely a 100% risk customer is to accept
    accept_rate_max=0.60,  # How likely a 0% risk customer is to accept
):
    ...
    # Define Continuous Acceptance Probability
    # Real behavior is non-linear. Loyalists stay high, Churners drop fast.
    # We use a Logistic Function (Sigmoid) to model this curve.

    k = 7  # Steepness: Higher = sharper drop-off (cliff)
    x0 = 0.85  # Midpoint: At what churn prob does acceptance drop to average?

    # Sigmoid Formula: Min + (Max - Min) / (1 + exp(k * (x - x0)))
    # When x (risk) is low, denominator is small -> Accept is Max
    # When x (risk) is high, denominator is huge  -> Accept is Min
    df_res["prob_accept"] = accept_rate_min + (
        (accept_rate_max - accept_rate_min) / (1 + np.exp(k * (df_res["y_prob"] - x0)))
    )

    # Calculate Expected Uplift before Sorting
    # We want to sort by "Who makes us the most money?", not "Who is most likely to leave?"
    # EV_Revenue = (Prob they are a churner) * (Prob they accept) * LTV
    # EV_Cost    = Cost_Contact + (Prob they accept * Cost_Offer)

    # Note: We use y_prob (prediction) for the Strategy (Sorting),
    # but we will use y_true (actuals) later for the Evaluation.
    df_res["expected_revenue"] = df_res["y_prob"] * df_res["prob_accept"] * ltv
    df_res["expected_cost"] = cost_contact + (df_res["prob_accept"] * cost_offer)

    df_res["expected_lift"] = df_res["expected_revenue"] - df_res["expected_cost"]

    # Sort by Expected Lift (Value)
    df_res = df_res.sort_values("expected_lift", ascending=False).reset_index(drop=True)

    # --- 2. Cumulative Calculations ---
    # Now that we are sorted by Value, we calculate the cumulative effect

    # "Actual" Profit (The Truth) - Used for the Report
    # We use y_true here because we are evaluating how the strategy WOULD have performed.
    df_res["realized_revenue"] = df_res["y_true"] * df_res["prob_accept"] * ltv
    df_res["realized_profit"] = df_res["realized_revenue"] - df_res["expected_cost"]

    df_res["cum_profit"] = df_res["realized_profit"].cumsum()
    df_res["cum_revenue"] = df_res["realized_revenue"].cumsum()
    ...
    # The cutoff is based on Expected Lift, not just Probability
    # But for the report, we can check the probability of the last person targeted
    optimal_prob_threshold = df_res.loc[max_profit_idx, "y_prob"]
    ...
    # Logic specific to the Targeted Group
    target_group = df_res.iloc[:optimal_customers]
    total_targeted = len(target_group)
    tp_count = row["cum_tp"]  # Actual Churners Captured
    fp_count = total_targeted - tp_count  # Loyalists Targeted (Waste)

    # Averages
    avg_risk = target_group["y_prob"].mean()
    avg_acceptance = target_group["prob_accept"].mean()

    # Waste Calculation (Mistake #3 Fixed: Loyalists accept MORE now)
    ...
    # Baselines
    # Compare against "Target Everyone" strategy
    profit_all = df_res.iloc[-1]["cum_profit"]
    roi_all = (profit_all / df_res.iloc[-1]["cum_cost"]) * 100
    ...
    y_pred_optimal = np.zeros(len(y))
    # We predict "1" (Target) for the top N customers based on our Value sorting
    y_pred_optimal[:optimal_customers] = 1
    ...
    # --- SECTION A: The Strategic & Financial Deep Dive  ---
    print(" STRATEGIC DECISION: Value-Based Optimization")  # Sorted by Value, not just Risk
    print("---------------------------------------------------------")
    print(f" Target Volume:           {int(total_targeted):,} customers")
    print(
        f" Coverage:                {total_targeted / total_customers:.1%} of total customer base"
    )
    print(f" Avg. Risk of Target:     {avg_risk:.1%}")
    print(f" Min Risk Targeted:       {optimal_prob_threshold:.1%} (approx)")
    ...
    print(f" ROI (Return on Spend):   {optimal_roi:.1f}%")
    print("---------------------------------------------------------\n")

    print(" BASELINE COMPARISON")
    print("---------------------------------------------------------")
    print(f" Profit if Target ALL:    {currency} {profit_all:,.0f} (ROI: {roi_all:.1f}%)")
    print(f" Profit Improvement:      {currency} {max_profit - profit_all:,.0f} vs Spray-and-Pray")
    print("---------------------------------------------------------\n")

    print(" EFFICIENCY & WASTE ANALYSIS")
    print("---------------------------------------------------------")
    print(f" Correctly Targeted:      {int(tp_count):,} (Churners Saved)")
    print(f" Incorrectly Targeted:    {int(fp_count):,} (Loyalists)")
    print(f" Cost of False Positives: {currency} {cost_of_fp:,.0f} (Waste)")
    print(
        f" Spend Efficiency:        For every {currency} 1 spent, we generate {currency} {row['cum_revenue'] / row['cum_cost']:.2f}"
    )
    print("---------------------------------------------------------\n")
    ...
    print(classification_report(y, y_pred_default, target_names=["Untargeted", "Targeted"]))
    print(f"\n>>> 2. Optimal Threshold ({optimal_prob_threshold:.4f}) - Business:")
    print(classification_report(y, y_pred_optimal, target_names=["Untargeted", "Targeted"]))
    ...
    ax1.axhline(profit_all, color="gray", ls="--", label="Target All")
    ax1.scatter(optimal_customers, max_profit, color="red", s=100, zorder=5)
    ax1.set_title("Profit Curve (Sorted by EV)", fontsize=12, fontweight="bold")
    ...
    ax2.plot(
        df_res.index / len(df_res), df_res["cum_tp"] / df_res["y_true"].sum(), color="blue", lw=2
    )
    ...
    # Plot 4: Confusion Matrix (Optimal)
    ax4 = fig.add_subplot(gs[1, 0])
    ConfusionMatrixDisplay.from_predictions(
        y, y_pred_optimal, ax=ax4, cmap="Greens", colorbar=False
    )
    ax4.set_title(
        f"Confusion Matrix at Optimal Threshhold {optimal_prob_threshold:.2f}",
        fontsize=12,
        fontweight="bold",
    )

    # Plot 5: Risk vs Acceptance (The new logic)
    ax5 = fig.add_subplot(gs[1, 1])
    # Scatter plot of a sample to show the relationship
    sample = df_res.sample(min(1000, len(df_res)))
    sns.scatterplot(x=sample["y_prob"], y=sample["prob_accept"], ax=ax5, alpha=0.3, color="purple")
    ax5.set_title("Risk vs. Acceptance (Modeled)", fontsize=12, fontweight="bold")
    ax5.set_xlabel("Churn Probability (Risk)")
    ax5.set_ylabel("Acceptance Probability")
    ax5.grid(True)

    # Plot 6: Targeted Group Probability Dist
    ax6 = fig.add_subplot(gs[1, 2])
    sns.histplot(target_group["y_prob"], bins=20, ax=ax6, color="orange")
    ax6.set_title("Risk Profile of Targeted Users", fontsize=12, fontweight="bold")
...
def comprehensive_churn_evaluation(
    ...
    df_res["prob_accept"] = np.where(
        df_res["y_prob"] > high_risk_threshold, acceptance_rate_high_risk, acceptance_rate_base
    )
    ...
    ax4 = fig.add_subplot(gs[1, 0])
    ConfusionMatrixDisplay.from_predictions(
        y, y_pred_default, ax=ax4, cmap="Blues", colorbar=False
    )
    ...
    ax5 = fig.add_subplot(gs[1, 1])
    ConfusionMatrixDisplay.from_predictions(
        y, y_pred_optimal, ax=ax5, cmap="Greens", colorbar=False
    )
    ax5.set_title(
        f"Confusion Matrix (Thresh={optimal_threshold:.2f})", fontsize=12, fontweight="bold"
    )
...
def run_sensitivity_analysis(
    ...
    for sc in scenarios:
        ...
        prob_accept = np.where(is_high_risk, base_rate * high_risk_decay, base_rate)
    ...
    for i, res in enumerate(results):
        ...
        plt.plot(
            x_axis,
            res["cum_profit_curve"],
            label=f"{res['Scenario']} (ROI: {res['ROI']:.1f}%)",
            color=colors[i],
            linewidth=2.5,
        )
    ...
    plt.title(
        f"Profit Impact of Different Offer Strategies ({currency})", fontsize=14, fontweight="bold"
    )
...
def analyze_prediction_errors(model, X, y, threshold=0.5, top_features=5):
    ...
    if "FP (False Alarm)" in grouped.index and "TP (Correct Catch)" in grouped.index:
        ...
        for feature in top_confusers.index:
            ...
            print(
                f"- {feature}: FP Avg ({val_fp:.2f}) is {pct_diff:+.1f}% vs Loyal Avg ({val_tn:.2f})"
            )
    ...
    if "FN (Missed Churn)" in grouped.index and "TN (Safe)" in grouped.index:
        ...
        for feature in top_hiders.index:
            ...
            print(
                f"- {feature}: FN Avg ({val_fn:.2f}) is {pct_diff:+.1f}% vs Caught Churners ({val_tp:.2f})"
            )
